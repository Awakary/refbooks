...
class ApiTestCase(TestCase):
    ...
    def test_get_refbooks_elements_status_code_equals_200(self):
        response = requests.get("http://localhost:8000/refbooks/10/elements/")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.headers['Content-Type'], "application/json")
        response_body = response.json()
    ...
    def test_get_refbooks_check_element_status_code_equals_200(self):
        response = requests.get("http://localhost:8000/refbooks/10/check_element/?code=CA01&value=Острый риносинусит")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.headers['Content-Type'], "application/json")
        response_body = response.json()
